    ...
    <>
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={cn(
            "flex max-w-fit fixed top-6 inset-x-0 mx-auto border border-white/[0.2] rounded-full bg-black/80 backdrop-blur-md shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-6 py-3 items-center justify-center space-x-4",
            className,
          )}
        >
          {/* Sparkles background */}
...
function FloatingDockNavText({ navItems, activeSection, smoothScrollTo }: { navItems: { name: string; link: string }[]; activeSection: string; smoothScrollTo: (id: string) => void }) {
  ...
  return (
    <div
      className="hidden md:flex items-center space-x-6 z-10"
      ...
      {navItems.map((navItem, idx) => {
        ...
        return (
          <motion.button
            ...
            style={{ scale }}
            className={cn(
              "relative transition-colors duration-200 text-base font-medium cursor-pointer bg-transparent border-none outline-none bebas-neue-regular",
              isActive ? "text-blue-400" : "text-neutral-50 hover:text-blue-400",
            )}
          >
            {navItem.name}
